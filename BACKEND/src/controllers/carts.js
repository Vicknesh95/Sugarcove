...
const addToCart = async (req, res) => {
  try {
    const { product_id, quantity, price, notes } = req.body;

    const cartItem = await pool.query(
      "SELECT * FROM carts WHERE user_id = $1 AND product_id = $2",
      [req.decoded.id, product_id]
    );
    ...
    if (cartItem.rows.length > 0) {
      ...
      const updatedCartItem = await pool.query(
        ...
        [addQuantity, req.decoded.id, product_id]
      );
...
const updateCart = async (req, res) => {
  try {
    ...
    const updatedCart = await pool.query(
      ...
      [quantity, notes, req.decoded.id, product_id]
    );
